...
class PlatformAPI{
  static Future<http.Response> platformResponse(Map<String, dynamic> body) async {
    return http.post(
      Uri.parse(port),
...
class ActionAPI extends PlatformAPI{
  static Future<dynamic> chkConnection() async{
    try{
      ...
      if (chkResponse.statusCode == 200) {
        ...
        Action action = Action.fromJson((jsonResponse as Map<String, dynamic>)["action"]);
        if (action.venueList.isEmpty) return "empty";
        for (var venue in action.venueList){ //удаляем пустые сеансы
          if (venue.actionEventList.any((element) => element.categoryLimitList.isNotEmpty)){
            venue.actionEventList.removeWhere((element) => element.categoryLimitList.isEmpty
      ...
      if (port.contains('bil24')){
        switch (zone){
          case 'test' : port = 'https://api2.bil24.pro:1240/json';
          case 'real' : port = 'https://api2.bil24.pro/json';
        }
      }
      return false;
    }
  ...
  static Future<dynamic> fetchAction() async{
    ...
    if (chk is bool){
     ...
     if (response.statusCode == 200) {
       ...
       Action action = Action.fromJson((jsonDecode(response.body) as Map<String, dynamic>)["action"]);
       if (action.venueList.isEmpty) return "empty";
       for (var venue in action.venueList){ //удаляем пустые сеансы
         if (venue.actionEventList.any((element) => element.categoryLimitList.isNotEmpty)){
           venue.actionEventList.removeWhere((element) => element.categoryLimitList.isEmpty
     ...
     else {throw Exception('Failed to get GET_ACTION_EXT response.');}
    }
  }
  ...
  static Future<AssignedSeatsProcessor> computeSchemeData(int actionEventId,
      String placementUri) async {
    ...
    AssignedSeatsProcessor parsedData = AssignedSeatsProcessor(getSchema: schemeData,
        getSeatList: seatListData, actionEventId: actionEventId, siData: si);
    return parsedData;
  }

  static Future<dynamic> checkKDP(int tempAccessCode) async{
    final successAuth = await PlatformAPI.platformResponse(<String, dynamic>{
      'fid': fid,
      'token': token,
      'command': 'AUTH'
    });
...
class UserAPI extends PlatformAPI{
  static User synthUser = const User(userId: 0, sessionId: '', email: '',
  ...
  static Future<int> auth(WidgetRef ref, String email) async {
    ...
    if (response.statusCode == 200) {
      String data = response.body;
      var jsonResponse = jsonDecode(data);
      synthUser = User(userId: jsonResponse['userId'],
          sessionId: jsonResponse['sessionId'], email: email,
          state: UserStates.processing, totalSeatsInReserve: 0);
  ...
  static bindEmail(WidgetRef ref, String email) async {
    ...
    if (successAuth == 0) {
      ...
      if (response.statusCode == 200) {
        String data = response.body;
        var jsonResponse = jsonDecode(data);
        print(jsonResponse['command'] + ' ' + jsonResponse['resultCode'].toString());
      } else {
  ...
  static Future<dynamic> confirmEmail(String code, User tempUser) async {
    ...
    if (response.statusCode == 200) {
      ...
      print(jsonResponse['command'] + ' ' + jsonResponse['resultCode'].toString());
      if (jsonResponse['resultCode'] == 0){
        return User(userId: jsonResponse['userId'],
            sessionId: jsonResponse['sessionId'], email: jsonResponse['email'],
            state: UserStates.authorized, totalSeatsInReserve: 0);
      } else {
        return jsonResponse['resultCode'];
      }
    } else {
  ...
  static Future<dynamic> getUserInfo(User user) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['resultCode'] == 0){
        ...
        return jsonResponse['command'] + ' ' + jsonResponse['resultCode'].toString();
      }
    } else {
      throw Exception('Failed to get CONFIRM_EMAIL response.');
    }
...
class CartAPI extends PlatformAPI{
  static Future<CartOrder> getCart(User user) async {
    ...
    if (response.statusCode == 200) {
      ...
      if ((jsonResponse['actionEventList'] as List<dynamic>).isNotEmpty){
        ...
        for(var item in jsonResponse['actionEventList']){
          for (var itemExt in item['seatList']){
            tempList.add(CartItem(categoryPriceName: itemExt['categoryPriceName'],
                day: item['day'], time: item['time'], price: itemExt['price'],
                seatId: itemExt['seatId'], currency: jsonResponse['currency'],
                tariffName: itemExt['tariffPlanName'],
                tariffPlanId: itemExt['tariffPlanId'],
                sector: itemExt['sector'],
                row: itemExt['row'], number: itemExt['number'],
                serviceCharge: itemExt['serviceCharge'],
                discountReason: itemExt["discountReason"],
                categoryPriceId: itemExt["categoryPriceId"])
            );
          }
          resultList.add(CartItemWrapper(
      ...
      cartOrder = CartOrder(totalSum: jsonResponse['totalSum'], currency: currency,
          totalServiceCharge: jsonResponse['totalServiceCharge'],
          time: jsonResponse['time'], bList: resultList,);
      return cartOrder;
    } else {
  ...
  static reservationHelper(User user, List categoryList) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse['resultCode'] == 101 || jsonResponse['resultCode'] == -1){
        return jsonResponse['description'];
      } else {
      }
    } else {
  ...
  static Future<dynamic> reserveGeneralAdmission(Map<int, List<CompositedCategory>> data, User user) async {
  ...
  static Future<dynamic> reserveSeats(String command, User user, int seatId,
    ...
    final response = await PlatformAPI.platformResponse(command == 'UN_RESERVE_ALL'
        ? (<String, dynamic>{
      'fid': fid,
      'token': token,
      'command': 'RESERVATION',
  ...
  static Future<dynamic> addPromoCodes(User user, List<String> promoCodes) async{
    Map<String, dynamic> requestBody = {
      'fid': fid,
      'token': token,
