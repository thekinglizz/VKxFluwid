...
final asyncSchemeProvider =
    AsyncNotifierProvider<SVMState, SchemeViewModel>(() {
  return SVMState();
});
    ...
    for (var sector in schemeData.sectorList) {
      for (var s in schemeData.siData.userSeatIdList) {
        if (sector.seatList.any((seat) => seat.bil24seatObj.seatId == s)) {
          Seat tempSeat = sector.seatList
              .firstWhere((seat) => seat.bil24seatObj.seatId == s);
          sList.add(tempSeat);
...
class SVMState extends AsyncNotifier<SchemeViewModel> {
  ...
  Future<dynamic> reserveSeat(
      Seat seat, User user, BuildContext context, String? fanId) async {
    List<Seat> userSeatList = state.value!.selectedSeats;
    dynamic reservationResult = await CartAPI.reserveSeats(
        'RESERVE', user, seat.bil24seatObj.seatId, null, null, fanId);
      ...
      if (context.mounted) {
        await showDialog<String>(
            ...
            builder: (BuildContext context) {
              return UserDialog(
                  userMessage: reservationResult.toString(),
                  redirectFlag: false);
            });
      }
    }
  ...
  Future<dynamic> reserveSeatWithTariff(
      Seat seat,
      User user,
    ...
    if (tariffMap.containsKey(seat)) {
      ...
      await CartAPI.reserveSeats(
          'UN_RESERVE', user, seat.bil24seatObj.seatId, null, null, null);
    }
  ...
  void unselectSeatOnScheme(int seatId) {
    ...
    userSeatList
        .removeWhere((element) => element.bil24seatObj.seatId == seatId);
    tempMap.removeWhere((key, value) => key.bil24seatObj.seatId == seatId);
    state = AsyncValue.data(state.value!.copyWith(
        selectedSeats: userSeatList,
        selectedTariffSeats: tempMap,
        categoryPriceFilter: state.value!.categoryPriceFilter));
  }

  void clearSelectedSeats() {
    state = AsyncValue.data(
        state.value!.copyWith(selectedSeats: [], selectedTariffSeats: {}));
  }

  void filterCategories(CategoryPriceObj? category) {
    state =
        AsyncValue.data(state.value!.copyWith(categoryPriceFilter: category));
  }
}
